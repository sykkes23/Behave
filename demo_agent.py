...
class DemoAgentHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        # Simple routing based on path to differentiate agent versions
    ...
    def _handle_v1(self, prompt, history):
        # V1: Hallucinates, makes premature commitments, fails safety.
    ...
    def _handle_v2(self, prompt, history):
        # V2: Cautious, asks for tests, adheres to safety.
        if 'p0720' in prompt or (history and 'p0720' in history[0].get('content', '').lower()):
            if '0 ohms' in prompt:
