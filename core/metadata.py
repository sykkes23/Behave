...
def get_git_info() -> Tuple[str, bool]:
    """
    Attempts to get the current Git commit hash and whether the tree is dirty.
    Degrades gracefully if Git is not available.
    Returns: (commit_hash, is_dirty)
    """
    try:
        # Get commit hash
...
def sanitize_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """Removes sensitive keys before hashing."""
...
def hash_dict(data: Dict[str, Any]) -> str:
    """Produces a deterministic hash of a dictionary."""
    if not data:
        return "unknown"
...
def hash_string(text: str) -> str:
    """Produces a deterministic hash of a string."""
    if not text:
        return "unknown"
