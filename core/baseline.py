...
def create_baseline(name: str, provider: str = None):
    ...
    # Query database for latest runs per test_id for the given provider
    # If provider is not specified, just get the latest of everything
    run_ids = []
    session_ids = []
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        # Test Runs
        if provider:
            # We need to extract provider from metadata_json, which is JSON. We can use LIKE as a hack or json_extract
            query = """
...
def load_baseline(name: str):
    ...
    # We load them back into TestResult and SessionResult objects
    ...
    # We will just return raw dicts for comparison since dataclass deserialization from deeply nested dicts can be tricky.
    # Wait, the user wants a compare engine, let's just return dicts.
