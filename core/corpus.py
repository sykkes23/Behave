...
class CorpusManager:
    ...
    def load_test_file(self, filepath: str) -> TestSpec:
        ...
        # Optional validation or dynamic hashing of content for version mismatch detection
        # We can strip the version field to hash the true logical definition
        ...
        # We could enforce that test_version matches content_hash, but for now we trust the version field
        # We'll just attach it as a dynamic field if we wanted to.
    ...
    def get_valid_tests(self) -> List[TestSpec]:
        """Returns only tests marked as VALID."""
    ...
    def compute_coverage(self) -> Dict[str, Any]:
        """
        Computes coverage metrics across the corpus.
        Returns counts for domains, principles, tags, and status.
        """
        coverage = {
